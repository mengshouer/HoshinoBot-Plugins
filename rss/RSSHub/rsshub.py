        ...
        if os.path.isfile(file_path + rss.name + ".json"):
                    ...
                    for item in change:
                        ...
                        try:
                            ...
                            msg = (
                                ...
                                + time.strftime(
                                    "%m{}%d{} %H:%M:%S",
                                    time.localtime(loc_time + 28800.0),
                                ).format("月", "日")
                            )
            ...
            async with httpx.AsyncClient(proxies=Proxy) as client:
                try:
                    ...
                    logger.error(
                        "出现异常，获取 " + rss.name + " 订阅xml失败！！！请检查订阅地址是否可用！  E:" + str(e)
                    )
                return []
    except BaseException as e:
    ...
    try:
        ...
        if rss.user_id:
            for id in rss.user_id:
                try:
                    await bot.send_msg(
                        message_type="private", user_id=id, message=str(msg)
                    )
                except Exception as e:
        ...
        if rss.group_id:
            for id in rss.group_id:
                try:
                    await bot.send_msg(
                        message_type="group", group_id=id, message=str(msg)
                    )
                except Exception as e:
            ...
            try:
                ...
                if cf.CLOSE_PIXIV_CAT and url.find("pixiv.cat") >= 0:
                    ...
                    req_json = requests.get(
                        "https://api.imjad.cn/pixiv/v1/?type=illust&id=" + info_list[0]
                    ).json()
                    if len(info_list) >= 2:
                        url = req_json["response"][0]["metadata"]["pages"][
                            int(info_list[1]) - 1
                        ]["image_urls"]["large"]
                    else:
...
# 处理正文
async def checkstr(
    rss_str: str, img_proxy: bool, translation: bool, only_pic: bool
) -> str:
    ...
    for img in doc_img.items():
        ...
        if len(img_path) > 0:
            if cf.IsLinux:
                ...
                rss_str = re.sub(
                    re.escape(str(img)),
                    r"[CQ:image,file=file:///" + str(img_path) + "]",
                    rss_str,
                )
        else:
    ...
    for video in doc_video.items():
        ...
        if len(img_path) > 0:
            if cf.IsLinux:
                ...
                rss_str = re.sub(
                    re.escape(str(video)),
                    r"视频封面：[CQ:image,file=file:///" + str(img_path) + "]",
                    rss_str,
                )
        else:
