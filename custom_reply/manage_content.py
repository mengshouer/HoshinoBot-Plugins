...
@on_command(
    "addCR",
    aliases=("CRadd", "addcr", "cradd", "add_custom_reply"),
...
async def add_custom_reply_content(session: CommandSession):
    ...
    if len(content) > 2:
        if os.path.exists("./data/custom_reply/HideCustomReplyList.json"):
            with open(
                "./data/custom_reply/HideCustomReplyList.json", "r", encoding="GB2312"
            ) as f:
                hidedata = json.load(f)
        hidedata[key] = ""
        with open(
            "./data/custom_reply/HideCustomReplyList.json", "w", encoding="GB2312"
        ) as f:
...
@on_command(
    "delCR",
    aliases=("CRdel", "delcr", "crdel", "del_custom_reply"),
    permission=SUPERUSER,
    only_to_me=True,
)
async def del_custom_reply_content(session: CommandSession):
    ...
    try:
        ...
        CRdata.data = data
        with open(
            "./data/custom_reply/CustomReplyData.json", "w", encoding="GB2312"
        ) as f:
            json.dump(data, f, indent=4, ensure_ascii=False)
        ...
        if os.path.exists("./data/custom_reply/HideCustomReplyList.json"):
            try:
                with open(
                    "./data/custom_reply/HideCustomReplyList.json",
                    "r",
                    encoding="GB2312",
                ) as f:
                    data = json.load(f)
...
@on_command(
    "/crlist", aliases=("/CRlist", "/CRLIST", "crlist", "CRLIST"), only_to_me=False
)
